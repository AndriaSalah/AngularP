...
//let formula = prompt('Enter your problem')
//formula = formula.replace(/\s+/g)
//console.log(formula)
debugger;
const operators = ['+','-','*','/']
let total = calculate('123+123')
let firstend
let secondend
let i = 0;
let y = i+1;
function calculate(str,startIndex = 0,prevValue = 0){
    let firstend = ''
    for (; startIndex < str.length; startIndex++) {
        if(!operators.includes(str[startIndex])){
        firstend += str[startIndex]//str.substring(0,startIndex)
        console.log(firstend + ' ' +startIndex + ' first end')
    }
    else break
}
    if(startIndex == str.length-1){
        return prevValue + parseInt(firstend) 
    }
   prevValue +=  calculate(str,startIndex+1,parseInt(firstend))
   return prevValue

// for (; i < formula.length; i++) {
//         if(operators.includes(formula[i])){
//         firstend = formula.substring(0,i)
//         console.log(firstend + ' ' +i + ' first end')
//         for (; y < formula.length; y++) {
//             if(operators.includes(formula[y]) ){
//                 secondend = formula.substring(i+1,y)
//                 console.log(secondend + ' ' + y + ' second end')
//                 break
//             }
//         }
//         if(char)
//         total = firstend + secondend
